...
const FeaturedProducts = () => {
  ...
  useEffect(() => {
    const fetchWatches = async () => {
      const query = `*[_type == "watch" && featured == true]{
        _id,
        "slug": slug.current,
        name,
        price,
  ...
  return (
    <section className="py-10">
      <h2 className="text-3xl font-bold text-center mb-6">
        Featu<span className="text-blue-600 dark:text-blue-400">red Products</span>
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 container mx-auto px-4">
        {loading ? (
          ...
          watches.map((watch) => <HoverableCard key={watch._id} watch={watch} />)
        ) : (
